...
import { addToWatchlist, removeFromWatchlist, getWatchlist } from "../services/movieService";
...
export const WatchlistProvider = ({ children }) => {
  ...
  const isMovieInWatchlist = (movieId) => {
    ...
    return watchlist.movies.some((movie) => movie._id?.toString() === movieId.toString());
  };
  ...
  const toggleWatchlist = async (movie) => {
    ...
    try {
      ...
      if (isMovieInWatchlist(movieId)) {
        ...
        setWatchlist((prev) => ({
          ...prev,
          movies: prev.movies.filter((m) => m._id.toString() !== movieId.toString()),
        }));
  ...
  return (
    <WatchlistContext.Provider value={{ watchlist, setWatchlist, isMovieInWatchlist, toggleWatchlist }}>
      {children}
